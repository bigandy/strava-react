...
class SingleRun extends React.Component{
	...
	componentDidMount() {
		$.getJSON(`https://www.strava.com/api/v3/activities/${this.props.params.id}/?per_page=${this.per_page}&access_token=${this.access_token}&before=${this.state.end_date}&after=${this.state.start_date}&callback=?`, (post) => {
			...
			const posts = (
					<div className='single-run'>
						...
						Distance: {distance.toFixed(2)}miles</p>
						 ...
						 <iframe height='405' width='590' frameBorder='0' allowTransparency='true' scrolling='no' src={`https://www.strava.com/activities/${this.props.params.id}/embed/${post.embed_token}`}></iframe>
					</div>
			);
	...
	// 			I am a single run with ID of { this.props.params.id } and title of {  this.props.title }
	// 		</section>
	// 	);
	// };
};
